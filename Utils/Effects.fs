...
let limiter_function (effect: Limiter_Effect) (sample: float32): float32 =
    let multiplier = 
        if effect.makeupGain then
            // Calculate make-up gain based on the lesser of the two limits so we don't produce clipping
...
// A method to access a partially applied version of the effect function
